...
function getVoiceList() {
  return ListVoice.map(item => `${item.replace(/(.+)-(.+)-(.+)Neural/, "$3 ($1-$2)")} ( ${item.split("-")[0]} )`);
}
  ...
  try {
    if (type === "list") {
      ...
      if (!query) {
        return res.status(400).json({
          success: false,
          message: "Parameter 'query' wajib diisi untuk 'generate'."
        });
      }
      const voiceData = await generateVoice(locale || "id-ID", voice || "id-ID-ArdiNeural", query);
      if (!voiceData) {
        throw new Error("Gagal menghasilkan suara.");
      }
      res.set("Content-Type", "audio/mp3");
      return res.status(200).send(voiceData);
    }
  } catch (error) {
