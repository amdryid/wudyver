...
class SnapSave {
  constructor(url) {
    this.url = url;
    this.baseUrl = "https://snapsave.app/action.php?lang=en";
    this.headers = {
  ...
  parseFsaveData(data) {
    try {
      ...
      throw new Error("Gagal memproses data SnapSave");
    }
  }
  async fetchData() {
    try {
      ...
      throw new Error("Gagal mengambil data dari SnapSave: " + error.message);
    }
  }
}
export default async function handler(req, res) {
  try {
    ...
    const snapsave = new SnapSave(url);
    const result = await snapsave.fetchData();
    res.status(200).json({
      result: result
    });
  } catch (error) {
