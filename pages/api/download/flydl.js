...
const download = async (videoUrl, format = "mp3", quality = "128") => {
  try {
...
export default async function handler(req, res) {
  ...
  if (!(url || format || quality)) return res.status(400).json({
    message: "No url, format, quality provided"
  });
  const result = await download(url, format, quality);
